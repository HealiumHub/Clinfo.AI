...
import { HashIcon, LinkIcon, PlusIcon, SearchIcon } from "lucide-react";
...
const Result: React.FC<ResultProps> = ({
  ...
  const handleSearchAnother = () => {
    setChatState(ChatStateEnum.INIT);
  };
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between">
            ...
            <Button className="w-fit" onClick={() => handleSearchAnother()}>
              <SearchIcon className="h-6 w-6 mr-1" /> Search another question
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          <CardDescription className="text-left flex flex-col gap-4">
            ...
            {result.synthesis.citations.map((citation) => (
              <div
                key={citation.url}
                className="flex flex-col gap-2 border-l-2 border-red-400 pl-2 hover:-translate-x-3 hover:cursor-pointer hover:opacity-80 transition-all"
              >
                <a
                  href={citation.url}
                  target="_blank"
                ...
                >
                  <p className="flex">
                    <HashIcon className="h-4 w-4 mr-1 dark:text-white text-black" />{" "}
                    {citation.text}
                  </p>
                </a>
              </div>
